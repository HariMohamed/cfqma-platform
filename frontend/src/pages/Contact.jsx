...
export function Contact() {
  ...
  const { t } = useI18n();
  const { settings, status: settingsStatus, error: settingsError } = useSiteContent();
    ...
    <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
        <div className="space-y-4">
          {settingsStatus === 'error' && <p className="rounded-md bg-red-50 p-4 text-sm font-semibold text-red-700">{settingsError}</p>}
          {phoneNumbers.length > 0 && <Info icon={<Phone />} text={phoneNumbers.join(' / ')} dir="ltr" />}
          {settings.email && <Info icon={<Mail />} text={settings.email} />}
          {settings.address && <Info icon={<MapPin />} text={settings.address} />}
          <div className="grid gap-3 sm:grid-cols-2">
            {settings.facebookUrl && (
              <a href={settings.facebookUrl} target="_blank" rel="noopener noreferrer" aria-label={t('common.facebook')} className="inline-flex items-center justify-center gap-2 rounded-md bg-white p-4 text-sm font-semibold shadow-sm ring-1 ring-ink/5 hover:text-craft dark:bg-white/10 dark:text-white dark:ring-white/10">
                <Facebook size={16} aria-hidden="true" /> {t('common.facebook')}
              </a>
            )}
            {settings.instagramUrl && (
              <a href={settings.instagramUrl} target="_blank" rel="noopener noreferrer" aria-label={t('common.instagram')} className="inline-flex items-center justify-center gap-2 rounded-md bg-white p-4 text-sm font-semibold shadow-sm ring-1 ring-ink/5 hover:text-craft dark:bg-white/10 dark:text-white dark:ring-white/10">
